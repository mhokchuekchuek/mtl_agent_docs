...
### `get(filter, with_vectors, limit) -> list[dict]`
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `filter` | dict | Metadata filter (e.g., `{"product_id": 123}`) |
| `with_vectors` | bool | Include embedding vectors (default: False) |
| `limit` | int | Maximum results (default: 1) |

**Returns**: List of results with id, metadata, and optionally vector
